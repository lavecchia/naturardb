...
def get_mol():
    ...
    filename="naturar_nid%s.mol"%(compound_id)
...
# get all molecule structures in NaturAr database 
def get_allmol():
    filename="naturar_all.mol"
    path=os.path.join(request.folder,'static/temp',filename)
    molfile = open(path,"w")
    response.headers['ContentType'] ="application/octet-stream";
    response.headers['Content-Disposition']="attachment; filename="+filename

    for compound in db().select(db.compound.id):        
        row = db(db.molstructure.compound_id==compound.id).select(db.molstructure.molstructure).first()
        try:
            molfile.write(row['molstructure'])
            molfile.write("\n\n")
        except:
...
# get all molecule structures in NaturAr database without semisynthetic
def get_allnatural():
    filename="naturar_all-with-out-semisynthetic.mol"
    path=os.path.join(request.folder,'static/temp',filename)
    molfile = open(path,"w")
    response.headers['ContentType'] ="application/octet-stream";
    response.headers['Content-Disposition']="attachment; filename="+filename

    for compound in db(db.compound.semisynthetic==False).select(db.compound.id):        
        row = db(db.molstructure.compound_id==compound.id).select(db.molstructure.molstructure).first()
        try:
            molfile.write(row['molstructure'])
            molfile.write("\n\n")
